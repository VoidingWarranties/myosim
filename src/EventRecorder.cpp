...
void EventRecorder::onPeriodic() {
  events_.queue.push_back(std::make_shared<PeriodicEvent>());
}

EventQueue EventRecorder::getEventQueue() {
  events_.num_myos = myo_indicies_.size();
  return events_;
}
...
void EventRecorder::onPair(myo::Myo* myo, uint64_t timestamp,
  ...
  events_.queue.push_back(std::make_shared<PairEvent>(getOrAddMyoIndex(myo),
                                                      timestamp,
                                                      firmwareVersion));
}
...
void EventRecorder::onUnpair(myo::Myo* myo, uint64_t timestamp) {
  ...
  events_.queue.push_back(std::make_shared<UnpairEvent>(getOrAddMyoIndex(myo),
                                                        timestamp));
}
...
void EventRecorder::onConnect(myo::Myo* myo, uint64_t timestamp,
  ...
  events_.queue.push_back(std::make_shared<ConnectEvent>(getOrAddMyoIndex(myo),
                                                         timestamp,
                                                         firmwareVersion));
}
...
void EventRecorder::onDisconnect(myo::Myo* myo, uint64_t timestamp) {
  ...
  events_.queue.push_back(std::make_shared<DisconnectEvent>(getOrAddMyoIndex(myo),
                                                            timestamp));
}
...
void EventRecorder::onArmSync(myo::Myo* myo, uint64_t timestamp, myo::Arm arm,
  ...
  events_.queue.push_back(std::make_shared<ArmSyncEvent>(getOrAddMyoIndex(myo),
                                                         timestamp, arm,
                                                         xDirection, rotation,
                                                         warmupState));
}
...
void EventRecorder::onArmUnsync(myo::Myo* myo, uint64_t timestamp) {
  ...
  events_.queue.push_back(std::make_shared<ArmUnsyncEvent>(getOrAddMyoIndex(myo),
                                                           timestamp));
}

void EventRecorder::onUnlock(myo::Myo* myo, uint64_t timestamp) {
  if (!(event_types_ & UNLOCK)) return;
  events_.queue.push_back(std::make_shared<UnlockEvent>(getOrAddMyoIndex(myo),
                                                        timestamp));
}

void EventRecorder::onLock(myo::Myo* myo, uint64_t timestamp) {
  if (!(event_types_ & LOCK)) return;
  events_.queue.push_back(std::make_shared<LockEvent>(getOrAddMyoIndex(myo),
                                                      timestamp));
}

void EventRecorder::onPose(myo::Myo* myo, uint64_t timestamp, myo::Pose pose) {
  if (!(event_types_ & POSE)) return;
  events_.queue.push_back(std::make_shared<PoseEvent>(getOrAddMyoIndex(myo),
                                                      timestamp, pose));
}
...
void EventRecorder::onOrientationData(myo::Myo* myo, uint64_t timestamp,
  ...
  events_.queue.push_back(std::make_shared<OrientationDataEvent>(getOrAddMyoIndex(myo),
                                                                 timestamp, rotation));
}
...
void EventRecorder::onAccelerometerData(myo::Myo* myo, uint64_t timestamp,
  ...
  events_.queue.push_back(std::make_shared<AccelerometerDataEvent>(getOrAddMyoIndex(myo),
                                                                   timestamp, accel));
}
...
void EventRecorder::onGyroscopeData(myo::Myo* myo, uint64_t timestamp,
  ...
  events_.queue.push_back(std::make_shared<GyroscopeDataEvent>(getOrAddMyoIndex(myo),
                                                               timestamp, gyro));
}

void EventRecorder::onRssi(myo::Myo* myo, uint64_t timestamp, int8_t rssi) {
  if (!(event_types_ & RSSI)) return;
  events_.queue.push_back(std::make_shared<RssiEvent>(getOrAddMyoIndex(myo),
                                                      timestamp, rssi));
}
...
void EventRecorder::onBatteryLevelReceived(myo::Myo* myo, uint64_t timestamp,
  ...
  events_.queue.push_back(std::make_shared<BatteryLevelReceivedEvent>(getOrAddMyoIndex(myo),
                                                                      timestamp, level));
}
...
void EventRecorder::onEmgData(myo::Myo* myo, uint64_t timestamp,
  ...
  events_.queue.push_back(std::make_shared<EmgDataEvent>(getOrAddMyoIndex(myo),
                                                         timestamp, emg));
}
...
void EventRecorder::onWarmupCompleted(myo::Myo* myo, uint64_t timestamp,
  ...
  events_.queue.push_back(std::make_shared<WarmupCompletedEvent>(getOrAddMyoIndex(myo),
                                                                 timestamp,
                                                                 warmupResult));
}
