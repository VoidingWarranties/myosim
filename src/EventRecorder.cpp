...
void EventRecorder::onPair(myo::Myo* myo, uint64_t timestamp,
  ...
  events_.queue.push_back(std::make_shared<PairEvent>(getMyoIndex(myo),
                                                      timestamp,
                                                      firmwareVersion));
}
...
void EventRecorder::onUnpair(myo::Myo* myo, uint64_t timestamp) {
  ...
  events_.queue.push_back(std::make_shared<UnpairEvent>(getMyoIndex(myo),
                                                        timestamp));
}
...
void EventRecorder::onConnect(myo::Myo* myo, uint64_t timestamp,
  ...
  events_.queue.push_back(std::make_shared<ConnectEvent>(getMyoIndex(myo),
                                                         timestamp,
                                                         firmwareVersion));
}
...
void EventRecorder::onDisconnect(myo::Myo* myo, uint64_t timestamp) {
  ...
  events_.queue.push_back(std::make_shared<DisconnectEvent>(getMyoIndex(myo),
                                                            timestamp));
}
...
void EventRecorder::onArmSync(myo::Myo* myo, uint64_t timestamp, myo::Arm arm,
  ...
  events_.queue.push_back(std::make_shared<ArmSyncEvent>(getMyoIndex(myo),
                                                         timestamp, arm,
                                                         xDirection, rotation,
                                                         warmupState));
}
...
void EventRecorder::onArmUnsync(myo::Myo* myo, uint64_t timestamp) {
  ...
  events_.queue.push_back(std::make_shared<ArmUnsyncEvent>(getMyoIndex(myo),
                                                           timestamp));
}

void EventRecorder::onUnlock(myo::Myo* myo, uint64_t timestamp) {
  if (!(event_types_ & UNLOCK)) return;
  events_.queue.push_back(std::make_shared<UnlockEvent>(getMyoIndex(myo),
                                                        timestamp));
}

void EventRecorder::onLock(myo::Myo* myo, uint64_t timestamp) {
  if (!(event_types_ & LOCK)) return;
  events_.queue.push_back(std::make_shared<LockEvent>(getMyoIndex(myo),
                                                      timestamp));
}

void EventRecorder::onPose(myo::Myo* myo, uint64_t timestamp, myo::Pose pose) {
  if (!(event_types_ & POSE)) return;
  events_.queue.push_back(std::make_shared<PoseEvent>(getMyoIndex(myo),
                                                      timestamp, pose));
}
...
void EventRecorder::onOrientationData(myo::Myo* myo, uint64_t timestamp,
  ...
  events_.queue.push_back(std::make_shared<OrientationDataEvent>(getMyoIndex(myo),
                                                                 timestamp, rotation));
}
...
void EventRecorder::onAccelerometerData(myo::Myo* myo, uint64_t timestamp,
  ...
  events_.queue.push_back(std::make_shared<AccelerometerDataEvent>(getMyoIndex(myo),
                                                                   timestamp, accel));
}
...
void EventRecorder::onGyroscopeData(myo::Myo* myo, uint64_t timestamp,
  ...
  events_.queue.push_back(std::make_shared<GyroscopeDataEvent>(getMyoIndex(myo),
                                                               timestamp, gyro));
}

void EventRecorder::onRssi(myo::Myo* myo, uint64_t timestamp, int8_t rssi) {
  if (!(event_types_ & RSSI)) return;
  events_.queue.push_back(std::make_shared<RssiEvent>(getMyoIndex(myo),
                                                      timestamp, rssi));
}
...
void EventRecorder::onBatteryLevelReceived(myo::Myo* myo, uint64_t timestamp,
  ...
  events_.queue.push_back(std::make_shared<BatteryLevelReceivedEvent>(getMyoIndex(myo),
                                                                      timestamp, level));
}
...
void EventRecorder::onEmgData(myo::Myo* myo, uint64_t timestamp,
  ...
  events_.queue.push_back(std::make_shared<EmgDataEvent>(getMyoIndex(myo),
                                                         timestamp, emg));
}
...
void EventRecorder::onWarmupCompleted(myo::Myo* myo, uint64_t timestamp,
  ...
  events_.queue.push_back(std::make_shared<WarmupCompletedEvent>(getMyoIndex(myo),
                                                                 timestamp,
                                                                 warmupResult));
}
